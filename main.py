...
def main():
    ...
    print("=" * 60)
    print("Extrator de Dados do SISREG III para Google Sheets")
    print("=" * 60)
    ...
    if not dependencias_ok:
        print("\nAVISO: Algumas dependências não estão instaladas.")
        print("O programa tentará usar métodos alternativos, mas pode haver problemas.")
    ...
    if not os.path.exists(config["pasta_pdfs"]):
        print(f"\nPasta de PDFs não encontrada: {config['pasta_pdfs']}")
        print(f"Criando pasta: {config['pasta_pdfs']}")
        os.makedirs(config["pasta_pdfs"])
    
    # Verificar se o arquivo de credenciais existe
    if not os.path.exists(config["arquivo_credenciais"]):
        print(f"\nAVISO: Arquivo de credenciais não encontrado: {config['arquivo_credenciais']}")
        print("Você precisará fornecer um arquivo de credenciais do Google para enviar dados para o Google Sheets.")
        
        # Perguntar se deseja continuar apenas com extração
        resposta = input("Deseja continuar apenas com a extração de dados (sem envio para Google Sheets)? (s/n): ")
        if resposta.lower() != 's':
            return
    
    # Processar PDFs
    print(f"\nIniciando processamento de PDFs na pasta: {config['pasta_pdfs']}")
    ...
    # Salvar dados em CSV local (backup)
    df = pd.DataFrame(dados_extraidos)
    csv_path = "dados_extraidos.csv"
    df.to_csv(csv_path, index=False)
    print(f"\nDados salvos localmente em: {csv_path}")
    
    # Verificar se deve enviar para Google Sheets
    if os.path.exists(config["arquivo_credenciais"]):
        # Configurar e enviar para Google Sheets
        print(f"\nConfigurando conexão com Google Sheets: {config['nome_planilha']}")
